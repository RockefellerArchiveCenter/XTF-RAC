<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
    ...
    <xsl:template name="myList">
        ...
        <xsl:if test="meta/level='file' or meta/level='item' or meta/type='mods'">
            ...
            <xsl:variable name="refid">
                <xsl:value-of select="substring-after($identifier, concat($callNo, '-'))"/>
            </xsl:variable>
            <xsl:variable name="container1">
                <xsl:choose>
                    <xsl:when test="contains(meta/containers[1], ',')">
                        <xsl:value-of
                            select="normalize-space(substring-before(meta/containers[1], ', '))"/>
                    </xsl:when>
                    <xsl:otherwise>
                        <xsl:value-of select="normalize-space(meta/containers[1])"/>
                    </xsl:otherwise>
                </xsl:choose>
            </xsl:variable>
            <xsl:variable name="container2">
                <xsl:if test="contains(meta/containers[1], ',')">
                    <xsl:value-of select="normalize-space(substring-after(meta/containers[1], ', '))"/>
                </xsl:if>
            </xsl:variable>
    ...
    <xsl:template name="myListEad">
        ...
        <xsl:variable name="refid">
            <xsl:value-of select="substring-after($identifier, concat($rootID, '-'))"/>
        </xsl:variable>
        <xsl:variable name="collectionTitle">
    ...
    <xsl:template name="myListCopies">
        <div class="overlay" id="myListCopies">
            <form id="duplicationForm" method="post" target="new"
                ...
                <div class="register"><input id="costagree" type="checkbox"/> I agree to pay the duplication costs for this request. See our <a href="https://rockarch.org/collections/access-and-request-materials/#duplication-services-and-fee-schedule" target="_blank" onClick="ga('send', 'event', 'My List', 'Fee Schedule', 'Duplication Dialog');">fee schedule</a>.</div>
                <div class="third">
                    <div class="form-group">
